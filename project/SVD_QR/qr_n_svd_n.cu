...
void fill(float *p, int tot_elems){
...
void readMatrixFromFile(float *p, int lda){
    ...
    //fill the matrix
    row=0;
    while ((getline(&line, &len, myFile) != -1) && row<ROWS) {
        col=0;
        do{
            word=strsep(&line,",");
            attr = atof(word);
...
int main(int argc, char *argv[])
{
    ...
    const int rows = ROWS;
    const int cols = COLS;
    const int total_iterations = TOTAL_ITERATIONS;
    
    printf("with rows=%d\t cols=%d\tn= %d \n", rows,   cols, total_iterations);
    ...
    */
    ...
    cudaStat1 = cudaMalloc((void**)&devInfo_QR, sizeof(long));
    assert(cudaSuccess == cudaStat1);
    ...
    /* ====================== Compute & allocate buffer for both QR & SVD ====================== 
    ...
    double Mflop_rate, Mflop_rate_QR, Mflop_rate_SVD;
    printf ("Time for the kernel: %f ms\n", time_SVD);
    Mflop_rate_QR = (double) 1e-6 * 2 * rows * cols * cols * n_iterations / time_QR;
    Mflop_rate_SVD = (double) 1e-6 * 4 * cols * cols * cols * n_iterations / time_SVD;

    Mflop_rate = (double) 1e-6 * total_iterations * (4*cols*cols*cols + 2*rows*cols*cols) / (time_QR + time_SVD);
    printf ("n_iterations = %d\n",n_iterations);
    printf ("Mflop/s: overall:%lf\t QR:%lf \t SVD:%lf \n", Mflop_rate, Mflop_rate_QR, Mflop_rate_SVD);
